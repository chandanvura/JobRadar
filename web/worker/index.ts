...
async function ingest(request:Request,env:Env){
  ...
  const statements=companyUpdates.length?[env.DB.prepare("UPDATE companies SET enabled=0,updated_at=CURRENT_TIMESTAMP")]:[];
